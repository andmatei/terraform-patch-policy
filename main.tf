...
resource "aws_iam_policy" "automation_admin" {
...
# TODO: Define the S3 for logs
resource "aws_s3_bucket" "command_logs" {
  bucket = "ssm-command-logs-${data.aws_region.current.name}-${data.aws_caller_identity.current.account_id}"
}
